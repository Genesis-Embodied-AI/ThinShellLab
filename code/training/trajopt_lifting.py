...
parser.add_argument('--mode', type=str, default="grad")
parser.add_argument('--render_option', type=str, default="Taichi")
args = parser.parse_args()

ti.init(ti.gpu, device_memory_fraction=0.5, default_fp=ti.f64, default_ip=ti.i32, fast_math=False,
        offline_cache=True, offline_cache_max_size_of_files=1024 ** 3,
        offline_cache_cleaning_policy='version')

from thinshelllab.task_scene.Scene_lifting import Scene, Body
...
renderer = Renderer(sys, "lifting", option=args.render_option)

now_reward = -100000
for ww in range(args.l, args.r):
    save_path = f"../imgs/traj_opt_lift_{ww}"
