...
parser.add_argument('--noregen', action="store_true", default=False)
parser.add_argument('--vertical_only', action="store_true", default=False)
...
from Scene_bounce import Scene, Body
...
sys = Scene(cloth_size=0.06, target=args.target)
sys.cloths[0].Kb[None] = 500.0
analy_grad = Grad(sys, tot_timestep, sys.elastic_cnt - 1, vertical_only=args.vertical_only)
...
for ww in range(args.l, args.r):
    save_path = f"../imgs/traj_opt_bounce_{ww}"
    ...
    sys.reset()
    sys.mu_cloth_elastic[None] = 0.2
    sys.cloths[0].k_angle[None] = 3.14
    ...
    for i in range(args.iter):
        ...
        tot_reward = sys.compute_reward(analy_grad)
        ...
        tt = analy_grad.get_loss_bounce(sys)
        tt_list.append(tt)
